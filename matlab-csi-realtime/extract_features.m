function [features, cell] = extract_features(abs_csi, frequency)
    function [f] = calcule_features(curve, frequency)
        c_mean = mean(curve);
        c_max = max(curve);
        c_min = min(curve);
...
%     %subcarriers = [1, 10, 20, 30, 40, 50, 56];
    ...
    for i=subcarriers
        ...
        for j=1:size(s_data, 1)
            slice_data = s_data(j,:);
            features = cat(1, features, calcule_features(slice_data, dec_sec_freq));
        end
    end
