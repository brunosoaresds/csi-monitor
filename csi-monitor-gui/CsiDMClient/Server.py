...
class Server:
    ...
    def __init__(self, port, senderUri, receiverUri):
        ...
        try:
            self.senderManager = DMClient(splitedSender[0], splitedSender[1], 'sender')
        except Exception, e:
            Log.error('Could not create DMClient of sender: ' + str(e))
            self.ready = False

        splitedReceiver = self.receiverUri.split(':', 1)
        try:
            self.receiverManager = DMClient(splitedReceiver[0], splitedReceiver[1], 'receiver')
        except Exception, e:
            Log.error('Could not create DMClient of receiver: ' + str(e))
            self.ready = False
    ...
    def runServer(self):
        ...
        Log.info("CsiDMClient server started!")
        while True:
