...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if isfield(handles, 'csiDMClientPid')
        pid = handles.csiDMClientPid;
        command = ['kill -9 ', pid];
        system(command);
    end
...
function handles = configureCsiDMClient(hObject, eventdata, handles)
    ...
    % if is not connected to csi device manager client, start and connect
    if isfield(handles, 'serverConnection') == 0
        disp('starting CSI Device Manager Client...');
        csiDMClientPort = randi([5000 6000], 1, 1);
        path = strrep(which(mfilename), [mfilename,'.m'], '');

        command = [path, 'CsiDMClient/Main.py --port=', num2str(csiDMClientPort), ...
        ' --receiver=', get(handles.csi_receiver, 'String'), ...
        ' --sender=', get(handles.csi_sender, 'String'), ...
        ' & echo $!'];
        [~, pid] = system(command);

        % Save the pid of CsiDMClient
        handles = setfield(handles, 'csiDMClientPid', pid);
        guidata(handles.csi_realtime_frame, handles);
        ...
        try
            t = tcpip('localhost', csiDMClientPort);
            ...
            disp(ME);
            disp('could not connect to CsiDMClient');
            connectionStatus = 'closed';
        end
...
% handles    structure with handles and user data (see GUIDATA)
    if strcmp(get(hObject, 'String'), 'Start')
        ...
        handles = configureCsiDMClient(hObject, eventdata, handles);
